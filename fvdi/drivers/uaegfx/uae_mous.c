...
static short backup_x, backup_y;
...
static long draw_mouse(Virtual *vwk, short x, short y)
{
	...
	c_expand_area(vwk, &mouse_mfdb, 0, 0, NULL, x, y, 16, 16, 2, pmouse->colour.background);

	/* Draw the data, transparent mode */
	mouse_mfdb.address = pmouse->data;
	c_expand_area(vwk, &mouse_mfdb, 0, 0, NULL, x, y, 16, 16, 2, pmouse->colour.foreground);
...
static void hide_mouse(Virtual *vwk)
{
	...
	{
		/* Restore the backup */
		c_blit_area(vwk, &mouse_backup_mfdb, 0, 0, NULL, backup_x, backup_y, 16, 16, 3);
...
static void show_mouse(Virtual *vwk, short x, short y)
{
	...
	c_blit_area(vwk, NULL, backup_x, backup_y, &mouse_backup_mfdb, 0, 0, 16, 16, 3);

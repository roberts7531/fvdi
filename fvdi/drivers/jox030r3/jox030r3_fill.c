                       ...
                       short *pattern, long colour, long mode, long interior_style)
{
	...
	if ((long)vwk & 1) {
		...
		if (x>=1024 | y>=768 ) return 0;

	//c_get_colours(vwk, colour, &foreground, &background);
	char buffer[50];
	switch(mode){
		case 1:	fillPattern((uint16_t)x,(uint16_t)y,(uint16_t)w,(uint16_t)h,pattern,colour&0xff,colour>>8,0,true);
		return 1;
		case 3:	fillPattern((uint16_t)x,(uint16_t)y,(uint16_t)w,(uint16_t)h,pattern,0,1,1,true);
